...
def create_new_speech_item(school_code,row):
    pkg_id = row['pkg_id']
    ...
    audio_full_file_name = os.path.join(file_tools.get_audio_data_dir_for_school(school_code),pkg_id,config.ECUBE_PKG_AUDIO_DIR,audio_file_name)
...
def get_speech_items(school_code,merged_school_audio_data_file):
    speech_items = []
    with open(merged_school_audio_data_file, mode='r', newline='') as file:
        reader = csv.DictReader(file)
        for row in reader:
            speech_item = create_new_speech_item(school_code,row)
