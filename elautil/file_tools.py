...
def get_audio_data_dir_for_school_byra(school_code):
    byra_data_dir = os.path.join(config.ELA_DATA_DIR,'byra')
    byra_school_data_dir = os.path.join(byra_data_dir,school_code)
    return byra_school_data_dir



def find_valid_audio_data_directories(data_source_format,school_data_dir):
    valid_data_directories = []
    # List all directories under school_data_dir
    directories = [name for name in os.listdir(school_data_dir) if os.path.isdir(os.path.join(school_data_dir, name))]
    ...
    if (data_source_format == 'BYRA'):
        valid_data_directories = directories
        #TODO more detailed validation

    return valid_data_directories

def get_audio_data_dir_for_school(school_code,data_source_format):
    audio_data_dir = None
    if data_source_format == 'ECUBE':
        audio_data_dir = get_audio_data_dir_for_school_ecube(school_code)
    elif data_source_format == 'BYRA':
        audio_data_dir = get_audio_data_dir_for_school_byra(school_code)
    else:
...
def get_audio_data_files(school_code,data_source_format):
    ...
    if data_source_format == 'ECUBE':
        ...
        package_dir_names = find_valid_audio_data_directories(data_source_format,ecube_school_data_dir)
        ...
        byra_school_data_dir = get_audio_data_dir_for_school_byra(school_code)
        package_dir_names = find_valid_audio_data_directories(data_source_format,byra_school_data_dir)
        for package_dir_name in package_dir_names:
            #audio_data_dir = os.path.join(byra_school_data_dir,package_dir_name)
            audio_data_file = os.path.join(byra_school_data_dir,package_dir_name,package_dir_name + _ela_csv_file_extension)
            if os.path.isfile(audio_data_file):
                audio_data_files.append(audio_data_file)
            else:
                print(f'skipping corrupted package {package_dir_name} because audio data file {audio_data_file} is missing')
...
def merge_school_audio_data_files_ecube(school_code,audio_data_file_list):
    combined_rows = []
    ...
    merged_output_file_dir = get_audio_data_dir_for_school(school_code)
...
def merge_school_audio_data_files_byra(school_code,audio_data_file_list):
    combined_rows = []
    #data_header = ['pkg_id','user_id','user_name','course_id','assignment_id','attempt_number','creation_time','file_name']
    data_header = ['pkg_id',
                'SubmissionDate',
                'startTime',
                'asr_collect_data-intro',
                'asr_collect_data-classorlevel',
                'asr_collect_data-studentid',
                'asr_collect_data-activityid',
                'asr_collect_data-student_recording',
                'meta-instanceID,KEY',
                'SubmitterID',
                'SubmitterName',
                'AttachmentsPresent',
    ...
    # Write combined rows to output CSV file
    merged_output_file_name = generate_csv_filename_by_timestamp(school_code)
    merged_output_file_dir = get_audio_data_dir_for_school_byra(school_code)
    merged_file = os.path.join(merged_output_file_dir,merged_output_file_name)
    with open(merged_file, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(data_header)
...
def merge_school_audio_data_files(school_code,data_source_format,audio_data_file_list):
    if (data_source_format == 'ECUBE'):
        merged_file, combined_rows = merge_school_audio_data_files_ecube(school_code,audio_data_file_list)
        return merged_file, combined_rows
    elif (data_source_format == 'BYRA'):
        merged_file, combined_rows = merge_school_audio_data_files_byra(school_code,audio_data_file_list)
        return merged_file, combined_rows
    else:
        print('ERROR! Invalid merged output file')
        return None, 0
